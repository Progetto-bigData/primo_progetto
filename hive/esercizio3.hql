...
SELECT historical_stock_prices.ticker, MAX(historical_stock_prices.data) AS data_max, MIN(historical_stock_prices.data) AS data_min
FROM    
    historical_stock_prices
WHERE YEAR(historical_stock_prices.data) = '2017'
GROUP BY historical_stock_prices.ticker, MONTH(historical_stock_prices.data);

CREATE TABLE close_max AS
SELECT historical_stock_prices.ticker, historical_stock_prices.close AS close_max, anno2017.data_max AS data_max
FROM 
    anno2017
    JOIN
    historical_stock_prices
...
SELECT historical_stock_prices.ticker, historical_stock_prices.close AS close_min, anno2017.data_min AS data_min
FROM 
    anno2017
    JOIN
    historical_stock_prices
...
SELECT close_max.ticker, (((close_max.close_max / close_min.close_min)-1)*100) AS var_mensile, MONTH(close_max.data_max) AS month
FROM 
    close_max
    JOIN
    close_min
...
SELECT v1.month AS month, v1.ticker AS first_ticker, v1.var_mensile AS first_var_mensile, v2.ticker AS second_ticker, v2.var_mensile AS second_var_mensile
FROM 
    varianza v1
    JOIN
    varianza v2
...
SELECT varianza_coppia.month, varianza_coppia.first_ticker, varianza_coppia.first_var_mensile, varianza_coppia.second_ticker, varianza_coppia.second_var_mensile
FROM 
    varianza_coppia
    JOIN
    coppie_buone
    ON (varianza_coppia.first_ticker = coppie_buone.first_ticker AND varianza_coppia.second_ticker = coppie_buone.second_ticker)
ORDER BY varianza_coppia.first_ticker, varianza_coppia.second_ticker, varianza_coppia.month
LIMIT 48;
